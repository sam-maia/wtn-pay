  ...
  return (
    ...
    >
      ...
      <div className="w-full">
        <input
          type="tel"
          placeholder="Móvil del auspiciador"
          required
          value={indicador}
          ...
          onBlur={async () => {
            ...
            if (data.existe) {
              setIndicadorStatus({ carregando: false, valido: true, mensagem: "Patrocinador encontrado ✔️" })
            } else {
              setIndicadorStatus({ carregando: false, valido: false, mensagem: "Número de patrocinador no registrado ❌" })
            }
          }}
      ...
      <div className="w-full">
        <input
          type="tel"
          placeholder="Número de su móvil"
          required
          value={indicado}
          ...
          onBlur={async () => {
            ...
            if (data.existe) {
              setIndicadoStatus({ carregando: false, existe: true, mensagem: "Este número ya está registrado ❌" })
            } else {
              setIndicadoStatus({ carregando: false, existe: false, mensagem: "Número válido para el registro ✔️" })
            }
          }}
          className={`w-full border rounded-lg p-3 ${
      ...
      <input type="email" placeholder="Su e-mail" required className="w-full border border-zinc-400 rounded-lg p-3" onChange={(e) => setEmail(e.target.value)} />
      <input type="email" placeholder="Confirme su e-mail" required className="w-full border border-zinc-400 rounded-lg p-3" onChange={(e) => setConfirmEmail(e.target.value)} />
      ...
      <input type="password" placeholder="Elija un PIN de 4 números" required maxLength="4" className="w-full border border-zinc-400 rounded-lg p-3 tracking-widest text-center" onChange={(e) => setPin(e.target.value)} />
